...
export function FloatingBookingTimer() {
  ...
  return (
    ...
    >
      <div
        className={`flex items-center gap-2.5 p-3 sm:p-3.5 rounded-2xl shadow-2xl border backdrop-blur-xl transition-all ${
          isCritical
      ...
      >
        ...
        <div
          className={`w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0 ${
            isCritical ? 'bg-red-600 text-white' : 'bg-primary/10 text-primary'
          }`}
        >
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-1.5 mb-0.5">
            <span className={`font-mono font-black text-xs px-2 py-0.5 rounded-lg whitespace-nowrap ${
              isCritical ? 'bg-red-600/80 text-white' : 'bg-primary/15 text-primary'
            }`}>
              ⏱ {timeFormatted}
            </span>
        ...
        <div className="flex items-center gap-1.5 flex-shrink-0 ml-1">
          <button
            ...
            className={`px-3 py-2 rounded-xl font-bold text-xs active:scale-95 transition-all flex items-center gap-1.5 shadow-sm whitespace-nowrap ${
              isCritical
          ...
          <button
            type="button"
            onClick={async () => {
              if (window.confirm('¿Seguro que deseas cancelar esta reserva y liberar la cancha?')) {
