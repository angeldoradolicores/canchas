...
export default function EditPitchPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!res.ok || !data.success) throw new Error(data.error || 'No se pudo actualizar la cancha.');
  ...
  const handleDeletePitch = async () => {
    ...
    try {
      ...
      if (!res.ok || !data.success) {
        throw new Error(data.error || 'No se pudo eliminar la cancha.');
      }

      router.push('/dashboard/pitches');
      router.refresh();
    } catch (err: any) {
      setDeleteError(err.message || 'Error al intentar eliminar la cancha.');
    } finally {
      setDeleting(false);
    }
  };
    ...
    <div className="max-w-3xl mx-auto pb-24 p-4">
          ...
          <div className="bg-card border border-border rounded-2xl p-6 space-y-4">
            <div>
              <h2 className="font-bold text-base flex items-center gap-2">
                <span>🌐</span> Redes Sociales de la Cancha <span className="text-xs font-normal text-muted-foreground">(Opcional)</span>
              </h2>
              <p className="text-xs text-muted-foreground mt-0.5">
                Conecta tus perfiles de redes sociales para que los clientes puedan visitarlos directamente desde la cancha.
              </p>
            </div>
            ...
            <div className="space-y-3.5 pt-1">
              ...
              <div className="space-y-1.5">
                ...
                <input
                  type="text"
                  placeholder="https://instagram.com/tucancha o @tucancha"
                  value={instagramUrl}
                  onChange={e => setInstagramUrl(e.target.value)}
                  className="w-full px-4 py-2.5 text-sm border border-border rounded-xl bg-background outline-none focus:border-[#E4405F] transition-colors"
                />
              </div>
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-bold text-muted-foreground uppercase flex items-center gap-2">
                  <div className="w-5 h-5 rounded-md bg-black flex items-center justify-center text-white shrink-0">
                ...
                <input
                  type="text"
                  placeholder="https://tiktok.com/@tucancha o @tucancha"
                  value={tiktokUrl}
                  onChange={e => setTiktokUrl(e.target.value)}
                  className="w-full px-4 py-2.5 text-sm border border-border rounded-xl bg-background outline-none focus:border-foreground transition-colors"
                />
              </div>
              ...
              <div className="space-y-1.5">
                ...
                <input
                  type="text"
                  placeholder="https://facebook.com/tucancha o tucancha"
                  value={facebookUrl}
                  onChange={e => setFacebookUrl(e.target.value)}
                  className="w-full px-4 py-2.5 text-sm border border-border rounded-xl bg-background outline-none focus:border-[#1877F2] transition-colors"
                />
              </div>
            </div>
          </div>
          ...
          <div className="bg-card border border-border rounded-2xl p-6 space-y-6">
            ...
            <div className="space-y-4">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 w-full max-w-full min-w-0">
                ...
                {/* Contenedor de controles adaptado para móviles */}
                <div className="flex items-center gap-2 w-full sm:w-auto min-w-0">
                  ...
                  <div className="relative flex-1 sm:flex-initial sm:min-w-[200px] min-w-0" ref={dropdownRef}>
                    ...
                    >
                      <div className="flex items-center gap-2 truncate min-w-0">
                        ...
                        <span className={`truncate ${newTimeInput ? 'text-foreground font-black' : 'text-muted-foreground text-xs'}`}>
                          {newTimeInput ? formatHourLabel(newTimeInput) : 'Seleccionar'}
                        </span>
                      </div>
                  ...
                  {/* Botón Agregar - Texto siempre visible y protegido contra saltos */}
                  <button
                    type="button"
                    onClick={() => {
                      if (newTimeInput) {
          ...
          <div className="bg-card border border-border rounded-2xl p-4 sm:p-6 space-y-5 w-full max-w-full min-w-0 shadow-sm">
            ...
            <div className="flex gap-2 w-full max-w-full min-w-0">
              <input
                ...
                className="flex-1 px-3 py-2 text-xs border border-border rounded-xl bg-background outline-none min-w-0"
              />
              <button
                type="button"
                onClick={() => {
            ...
            {/* Grid de imágenes tal cual lo tenías (2 columnas en móvil, 3 en sm) */}
            {mediaItems.length > 0 ? (
      ...
      <div className="mt-12 p-5 border border-red-500/20 bg-red-500/5 rounded-2xl flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <button
          ...
          className="px-4 py-2.5 bg-red-600 hover:bg-red-700 text-white text-xs font-bold rounded-xl flex items-center justify-center gap-2 transition-all shadow-sm shrink-0 cursor-pointer active:scale-95"
        >
          <Trash2 size={14} />
      ...
      {showDeleteModal && (
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-background border border-border rounded-2xl max-w-md w-full p-6 space-y-4 shadow-xl animate-in fade-in zoom-in-95">
            ...
            {deleteError && (
              <div className="p-3 bg-red-500/10 border border-red-500/30 text-red-600 dark:text-red-400 text-xs rounded-xl flex items-center gap-2">
            ...
            <div className="flex gap-3 pt-2">
              <button
                ...
                disabled={deleting}
                className="flex-1 py-2.5 border border-border rounded-xl text-xs sm:text-sm font-semibold bg-secondary hover:bg-secondary/80 text-foreground transition-all cursor-pointer"
              >
                Cancelar
              </button>
              <button
                ...
                disabled={deleting}
                className="flex-1 py-2.5 bg-red-600 hover:bg-red-700 text-white rounded-xl text-xs sm:text-sm font-bold flex items-center justify-center gap-2 transition-all disabled:opacity-60 shadow-sm cursor-pointer active:scale-98"
              >
